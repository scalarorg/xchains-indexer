...
func (p *SignCommandsParser) ParseMessage(sdkMsg sdkTypes.Msg, messageLog *indexerTxTypes.LogMessage, cfg config.IndexConfig) (*any, error) {
	parsedMsg, ok := sdkMsg.(*chainsTypes.SignCommandsRequest)
	if !ok {
		return nil, errors.New("not a SignCommandsRequest message request")
	}
	var parsedValue any = &SignCommandsMsg{
		Type:   MSG_SCALAR_CHAINS_SIGN_COMMANDS_REQUEST,
...
func (p *SignCommandsParser) IndexMessage(dataset *any, db *gorm.DB, message models.Message, messageEvents []parsers.MessageEventWithAttributes, cfg config.IndexConfig) error {
	config.Log.Debugf("SignCommandsParser# IndexMessage# message: %++v, dataset: %T", message, *dataset)
	...
	if err != nil {
		config.Log.Debugf("SignCommandsParser# Failed to save message %v", err)
	}
	...
	if err == nil {
		...
		if err != nil {
			config.Log.Debugf("SignCommandsParser# Failed to save message event %v", err)
		}
	} else {
		config.Log.Debugf("SignCommandsParser# Failed to marshal message event %v", err)
		return err
	}
	return nil
}
