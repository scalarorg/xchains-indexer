...
func (p *BatchRequestParser) ParseMessage(sdkMsg stdTypes.Msg, messageLog *txTypes.LogMessage, cfg config.IndexConfig) (*any, error) {
	parsedMsg, ok := sdkMsg.(*auxiliaryTypes.BatchRequest)
	if !ok {
		return nil, errors.New("not a BatchRequest")
	}

	var parsedValue any = &BatchRequestMsg{
		Type:     MSG_SCALAR_AUXILIARY_BATCH_REQUEST,
...
func (p *BatchRequestParser) IndexMessage(dataset *any, db *gorm.DB, message models.Message, messageEvents []parsers.MessageEventWithAttributes, cfg config.IndexConfig) error {
	config.Log.Debugf("BatchRequestParser# IndexMessage# message: %++v, dataset: %T", message, *dataset)
	parsedMsg, ok := (*dataset).(*BatchRequestMsg)
	if !ok {
