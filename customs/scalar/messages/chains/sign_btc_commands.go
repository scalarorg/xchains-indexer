...
func (p *SignBtcCommandsParser) ParseMessage(sdkMsg sdkTypes.Msg, messageLog *indexerTxTypes.LogMessage, cfg config.IndexConfig) (*any, error) {
	parsedMsg, ok := sdkMsg.(*chainsTypes.SignBtcCommandsRequest)
	if !ok {
		return nil, errors.New("not a SignBtcCommandsRequest message request")
	}
	var parsedValue any = &SignCommandsMsg{
		Type:   MSG_SCALAR_CHAINS_SIGN_BTC_COMMANDS_REQUEST,
...
func (p *SignBtcCommandsParser) IndexMessage(dataset *any, db *gorm.DB, message models.Message, messageEvents []parsers.MessageEventWithAttributes, cfg config.IndexConfig) error {
	config.Log.Debugf("SignBtcCommandsParser# IndexMessage# message: %++v, dataset: %T", message, *dataset)
	...
	if err != nil {
		config.Log.Debugf("SignBtcCommandsParser# Failed to save message %v", err)
	}
	...
	if err == nil {
		...
		if err != nil {
			config.Log.Debugf("SignBtcCommandsParser# Failed to save message event %v", err)
		}
	} else {
		config.Log.Debugf("SignBtcCommandsParser# Failed to marshal message event %v", err)
		return err
	}
	return nil
}
